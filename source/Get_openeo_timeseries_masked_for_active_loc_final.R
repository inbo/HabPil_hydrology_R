...
required_packages <- c("sf", "openeo", "ggplot2", "readr", "terra", "jsonlite", "geojsonsf")
...
plot_points_wgs84 <- st_read("output/Watina/meetpunten_Dijlevallei_actief2018_WGS84.geojson")
...
compute_and_save_ndwi <- function(index_name, band1, band2, filename_prefix) {
  ...
  job = create_job(graph=result, title = index_name, description = paste("Exporting", index_name, "for Dijlevallei"))
  start_job(job=job)
...
job_mcfeeters <- compute_and_save_ndwi("NDWI_McFeeters", 1, 2, "NDWI_McFeeters_Dijle_points")
job_xu <- compute_and_save_ndwi("NDWI_Xu", 1, 3, "NDWI_Xu_Dijle_points")
job_gao <- compute_and_save_ndwi("NDWI_Gao", 2, 3, "NDWI_Gao_Dijle_points")


list_jobs()

download_results(job=job_mcfeeters, folder = "output/NDWI_McFeeters/Dijlevallei/masked")
download_results(job=job_xu, folder = "output/NDWI_Xu/Dijlevallei/masked")
download_results(job=job_gao, folder = "output/NDWI_Gao/Dijlevallei/masked")
